                                  ...
                                  double absolute_min_area)
{
    ...
    for (int i = 1; i < n_components; ++i)
    {
        ...
        const bool touches_border = bx == 0 || by == 0
                                 || bx + bw == image_width
                                 || by + bh == image_height;
        if (touches_border)
            continue;
        ...
        cv::circle(disk, local_center,
                   static_cast<int>(approximate_radius) - 2, 255, -1);
...
std::vector<Tip> find_triangles(const cv::Mat& img_cleaned)
{
    ...
    for (int i = 1; i < n_components; ++i)
    {
        ...
        Tip tip;
        tip.centroid = cv::Point2d(centroids.at<double>(i, 0),
...
std::optional<cv::Point> best_neighbor(int cx, int cy, const cv::Mat& img,
...
std::pair<std::vector<cv::Point>, std::optional<int>> follow_line(
    const cv::Mat& img, const cv::Point2d& start_pt, cv::Point2d direction,
    const std::vector<State>& states,
    std::optional<int> origin_dest_index = std::nullopt, int max_steps = 2000,
    int smoothing_window = 5, int min_steps_before_return = 8)
{
    ...
    for (int step = 0; step < max_steps; ++step)
    {
        ...
        const std::optional<int> hit =
            reached_state(current.x, current.y, states, step,
                                       ...
                                       const cv::Mat& img_clean_final)
{
    ...
    for (const Tip& triangle : triangles)
    {
        ...
        const double init_norm =
            std::sqrt(init_dx * init_dx + init_dy * init_dy);
        const cv::Point2d init_dir =
            init_norm > 0.0
                ? cv::Point2d(init_dx / init_norm, init_dy / init_norm)
        ...
        auto [chemin, source_index] =
            follow_line(img_suivi, cv::Point2d(base_x, base_y), init_dir,
                        states, tip.dest);
        if (!source_index)
        {
                       ...
                       const std::vector<Arrow>& arrows)
{
    ...
    for (const Arrow& arrow : arrows)
    {
        for (const cv::Point& p : arrow.chemin)
        {
            ...
            {
                const std::int32_t label_id =
                    labels_image.at<std::int32_t>(p.y, p.x);
                if (label_id != 0)
