  ...
  for (int i = 1; i < n_components; ++i) {
    ...
    std::vector<Tip> find_triangles(const cv::Mat &img_cleaned) {
      ...
      for (int i = 1; i < n_components; ++i) {
        ...
        if (circularity > 0.98)
          continue;
        std::cerr << "comp " << i << ": aire=" << component_area
                  << " bbox=(" << bx << "," << by << "," << bw << "," << bh << ")"
                  << " bord=" << (bx == 0 || by == 0 || bx + bw == image_width
                                  || by + bh == image_height)
                  << "perimeter" << perimeter
                  << "contours" << contours.empty() 
    ...
    for (const auto& t : tips)
        std::cerr << "  aire=" << t.area << " centroid=(" << t.centroid.x << "," << t.centroid.y << ")\n";
      split_merged_tips(tips);

std::cerr << "APRES split: " << tips.size() << " tips\n";
for (const auto& t : tips)
  std::cerr << "  aire=" << t.area << " centroid=("
            << t.centroid.x << "," << t.centroid.y << ")\n";
      return tips;
    }
...
std::vector<cv::Point> branch_candidates(
    int cx, int cy, const cv::Mat& img,
    const std::set<std::pair<int, int>>& visited,
    const cv::Point2d& direction, double min_align, double gap_deg)
{
    const int height = img.rows;
    const int width = img.cols;

    struct Cand { double angle; cv::Point pt; double align; };
    std::vector<Cand> cands;
    for (const auto& [dx, dy] : kNeighbors)
    {
        const int nx = cx + dx, ny = cy + dy;
        if (!(0 <= ny && ny < height && 0 <= nx && nx < width)) continue;
        if (img.at<std::uint8_t>(ny, nx) == 0) continue;
        if (visited.count({nx, ny})) continue;
        const double vx = nx - cx, vy = ny - cy;
        const double n = std::sqrt(vx * vx + vy * vy);
        if (n == 0.0) continue;
        const double align = (vx * direction.x + vy * direction.y) / n;
        if (align < min_align) continue;  // ecarte perpendiculaire / arriere
    ...
    bool open = false;
    auto flush = [&]() { if (open) reps.push_back(cluster_best_pt); };
...
std::vector<BranchResult> follow_line_branches(
    const cv::Mat& img, const cv::Point2d& start_pt, cv::Point2d initial_dir,
    const std::vector<State>& states, std::optional<int> origin_dest_index,
    int max_steps = 2000, int smoothing_window = 5,
    int min_steps_before_return = 8, double fork_min_align = 0.15,
    double fork_gap_deg = 20.0, int max_branches = 4)
{
        ...
        || img.at<std::uint8_t>(sy, sx) == 0)
    {
        ...
        for (int y = 0; y < height; ++y)
        {
            ...
            for (int x = 0; x < width; ++x)
            {
                ...
                const double d = (x - start_pt.x) * (x - start_pt.x)
                               + (y - start_pt.y) * (y - start_pt.y);
                if (d < best_d) { best_d = d; sx = x; sy = y; found = true; }
            }
        }
        if (!found) return {};
    }
    ...
    while (!stack.empty())
    {
        ...
        for (int step = 0; step < max_steps; ++step)
        {
            ...
            const std::optional<int> hit = reached_state(
                current.x, current.y, states,
                static_cast<int>(f.chemin.size()),
            ...
            const std::vector<cv::Point> reps = branch_candidates(
                current.x, current.y, img, visited, direction,
    ...
    AdjacencyResult build_adjacency_matrix(const std::vector<State> &states,
      ...
      for (const Tip &triangle : triangles) {
        ...
        std::vector<BranchResult> branches =
            follow_line_branches(img_suivi, cv::Point2d(base_x, base_y),
                                 init_dir, states, tip.dest);
        if (branches.empty())
        {
