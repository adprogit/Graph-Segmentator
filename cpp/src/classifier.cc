...
void KNNClassifier::fit(Model model)
{
...
std::vector<int> KNNClassifier::k_nearest_indices(
                                ...
                                const std::vector<float>& distances) const
{
    if (weighted_)
    {
        ...
        for (int idx : neighbor_indices)
        {
            ...
            it->second += 1.0 / (static_cast<double>(
                                     distances[static_cast<std::size_t>(idx)])
                                 + 1e-8);
        }
...
std::optional<std::string> KNNClassifier::predict_one(const float* x) const
{
    ...
    {
        const float min_dist = *std::min_element(distances.begin(),
                                                 distances.end());
...
std::optional<std::string> KNNClassifier::predict(
